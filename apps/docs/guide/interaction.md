...
- **矩形** — 四角圆角手柄  
- **星形** — 尖角数（✦，上下拖增减）与圆角手柄，锚定在轮廓旁，不随 tip 数飞出  
- **圆弧文字** — 半径 / 中心等手柄  
...
| 字段 | 含义 |
|------|------|
| `snapEnabled` | 端点 / 网格等绘制吸附（含粉色网格引导线） |
| `alignEnabled` | 拖拽 AABB 对象对齐（最佳一条引导线） |
| `angleStepDeg` | 旋转步长（默认 15°；Shift=45°） |
| `panDamping` | 平移惯性阻尼 |

Playground「设置」可分别开关对象对齐 / 网格吸附 / 角度吸附。
